...
class LKAS:
    ...
    def window_search(self, binary_line):
        ...
        # 각 열의 값들 다 더한 것의 배열을 히스토그램으로 정의
        ...
        for window in range(nwindows):
            ...
            if left_x_current != 0:
                cv2.rectangle(
                    out_img,
                    (win_x_left_low, win_y_low), #(좌측 x, 상단 y(사람 시점))
                    (win_x_left_high, win_y_high), #(우측 x, 하단 y(사람 시점))
                    (0, 255, 0),
                    int(2)
                )
            # 우측 차선
            if right_x_current != midpoint:
                cv2.rectangle(
                    out_img,
                    (win_x_right_low, win_y_low), #(좌측 x, 상단 y(사람 시점))
                    (win_x_right_high, win_y_high), #(우측 x, 하단 y(사람 시점))
                    (0, 0, 255),
                    int(2)
                )

            # x축 index 모음, y축 index 모음은 서로 대응, ex) x=[1,2,3,4,5,6], y=[1,2,3,4,5,6]
            # => (1,1)은 0, (2,2) 1, 즉 x, y의 인덱스를 세트로 생각
            # 왼쪽 오른쪽 영역의 각 픽셀이 각 영역의 window안에 있는 경우 픽셀의 index를 저장합니다.
            good_left_idx = (
            ...
            # window 안에 존재하는 픽셀 리스트
            left_lane_idx.append(good_left_idx)
            right_lane_idx.append(good_right_idx)

            # window내 설정한 pixel개수 이상이 탐지되면, 차선 중앙을 픽셀들의 x 좌표 평균으로 업데이트 합니다. => window x 위치 변경
    ...
    def img_CB(self, data):
        ...
        power = [(center[0,0]-self.img_x//2)*0.1,
                 (center[1,0]-self.img_x//2)*0.2,
                 (center[2,0]-self.img_x//2)*1.0,
                 (center[3,0]-self.img_x//2)*0.2,
                 (center[4,0]-self.img_x//2)*0.2]
